  ...
  data() {
    return {
      ...
      queryList: {
        ...
        yearId: "",
        searchStr: "",
        projectTypeId: localStorage.getItem("projectTypeId4"),
      },
      yearOptions: [], // 年份下拉列表值
      ...
      addForm: {
        ...
        projectTypeId: localStorage.getItem("projectTypeId4"), //项目类别 projectTypeName
        isFinish: 0,
      },
      isFinishOptions: [
        {
    ...
    getProjectTypeList() {
      getProjectTypeList().then((res) => {
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].typeName == "装备技术基础项目") {
            localStorage.setItem("projectTypeId4", res.data[i].projectTypeId);
          }
        }
      });
    },
