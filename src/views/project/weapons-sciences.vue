    ...
    <div class="tools">
      ...
      <el-dialog title="修改" :visible.sync="editDialogVisible" width="50%">
        ...
        >
          ...
          <el-form-item label="是否结题" prop="isFinish">
            <el-select v-model="eidtForm.isFinish" placeholder="请选择">
              <el-option
                v-for="item in isFinishOptions"
                :key="item.value"
                :label="item.label"
  ...
  data() {
    return {
      ...
      queryList: {
        ...
        yearId: "",
        searchStr: "",
        projectTypeId: localStorage.getItem("projectTypeId7"),
      },
      yearOptions: [], // 年份下拉列表值
      ...
      addForm: {
        ...
        projectTypeId: localStorage.getItem("projectTypeId7"), //项目类别 projectTypeName
        isFinish: 0,
      },
      isFinishOptions: [
        {
    ...
    getProjectTypeList() {
      getProjectTypeList().then((res) => {
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].typeName == "新立武器装备预研项目") {
            localStorage.setItem("projectTypeId7", res.data[i].projectTypeId);
          }
        }
      });
    },
