  ...
  <div class="approve">
    ...
    <div class="tools">
      ...
      <div class="tools-btn">
        ...
        <div>  <el-button   style="margin-left:10px;" type="primary" plain><a
          href="/static/科研项目.xlsx" download>下载模板</a></el-button></div>
      ...
      <el-dialog title="添加" :visible.sync="addDialogVisible" width="50%">
        ...
        >
          <el-form-item label="选择单位" prop="attrOrginId">
            <el-select  class="inner-input" v-model="addForm.attrOrginId" placeholder="请选择单位">
              <el-option
          ...
          <el-form-item label="选择年份" prop="yearId">
            <el-select  class="inner-input" v-model="addForm.yearId" placeholder="请选择年份">
              <el-option
                v-for="item in yearOptions"
                :key="item.yearId"
                :label="item.yearName"
          ...
          <el-form-item label="是否结题" prop="isFinish">
            <el-select  class="inner-input" v-model="addForm.isFinish" placeholder="请选择">
              <el-option
      ...
      <el-dialog title="修改" :visible.sync="editDialogVisible" width="50%">
        ...
        >
          <el-form-item label="选择单位" prop="attrOrginId">
            <el-select  class="inner-input" v-model="eidtForm.attrOrginId" placeholder="请选择单位">
              <el-option
                v-for="item in attrOrginOptions"
                :key="item.id"
                :label="item.attrOrginName"
          ...
          <el-form-item label="选择年份" prop="yearId">
            <el-select  class="inner-input" v-model="eidtForm.yearId" placeholder="请选择年份">
              <el-option
                v-for="item in yearOptions"
                :key="item.yearId"
                :label="item.yearName"
          ...
          <el-form-item label="是否结题" prop="isFinish">
            <el-select  class="inner-input" v-model="eidtForm.isFinish" placeholder="请选择">
              <el-option
                v-for="item in isFinishOptions"
                :key="item.value"
                :label="item.label"
  ...
  data() {
    return {
      ...
      queryList: {
        ...
        yearId: "",
        searchStr: "",
        projectTypeId: localStorage.getItem("projectTypeId2"),
      },
      yearOptions: [], // 年份下拉列表值
      ...
      addForm: {
        ...
        projectTypeId: localStorage.getItem("projectTypeId2"), //项目类别 projectTypeName
        isFinish: 0,
      },
      isFinishOptions: [
        {
    ...
    getProjectTypeList() {
      getProjectTypeList().then((res) => {
        for (let i = 0; i < res.data.length; i++) {
          if (res.data[i].typeName == "国家社会科学基金项目") {
            localStorage.setItem("projectTypeId2", res.data[i].projectTypeId);
          }
        }
      });
    },
